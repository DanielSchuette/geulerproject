...
// FindPrimeNumber takes an integer n and returns the n'th
// prime number.
...
loop:
	for {
		...
		// test if current number is evenly divisible by
		// an previous number (and thus, not a prime number)
		for i := 2; i < number; i++ {
		...
		// if inner loop does not continue to outer loop,
		// increment prime number counter
		counter++

		// if counter reached the desired n,
