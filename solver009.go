...
// PythagoreanTriplet finds the pythagorean triplet (see
// `main/problem9.go') takes a float64 n as its input and
// returns a, b, c  for which a + b + c = n or an error. The
// output of PythagoreanTriplet is not sorted!
func PythagoreanTriplet(n float64) (float64, float64, float64, error) {
	...
	// an outer loop checks variable whereas an inner
	// loop checks all variables b for which 0 < b < a;
	// negative numbers are not allow and not checked
